...
def produce_output_data_from_input_data
  ...
  raw_input['rental_modifications'].each do |rental_data|
    ...
    new_actions.each do |new_action|
      ...
      # here if amount negative & who is not 'driver' then its debit else credit
      # if each_action_result['amount'] < 0.0 && each_action_result['who'] != 'driver'
      #   each_action_result['type'] = 'debit'
      # elsif each_action_result['amount'] < 0.0 && each_action_result['who'] == 'driver'
      #   each_action_result['type'] = 'credit'
      ...
      # here if amount positive & who is 'driver' then its debit else credit
      # elsif each_action_result['amount'] > 0.0 && each_action_result['who'] != 'driver'
      #   each_action_result['type'] = 'credit'
      # elsif each_action_result['amount'] > 0.0 && each_action_result['who'] == 'driver'
      #   each_action_result['type'] = 'debit'
      ...
      # each_action_result['type'] = new_action['type'] # to be changed later on
      each_action_result['amount'] = rental_modification_amount.abs
      change_actions << each_action_result
    end
    # p change_actions
    Pry::ColorPrinter.pp(change_actions)

    # NOTE : we'll take care of the sign of the amount later (the sign impact the type credit or debit)
...
def prepare_intermediary_result_hash_of_data(rental_id, actions) # Change arguments to give a hash, easier
  res = {}
  res['id'] = rental_id
  res['actions'] = actions
  return res
